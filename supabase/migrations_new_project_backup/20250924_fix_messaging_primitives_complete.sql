...
-- Ensure constraint exists before backfill
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'conversations_unique_participants'
  ) THEN
    ALTER TABLE conversations ADD CONSTRAINT conversations_unique_participants UNIQUE (participant_one_id, participant_two_id);
  END IF;
END $$;
