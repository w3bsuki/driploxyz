<script lang="ts">
  ...
  import { createLazyLoader, loadComponent } from '$lib/utils/performance';
  import { searchProducts } from '$lib/remote/products.remote';
  ...
  async function loadComponents() {
    ...
    try {
      ...
      VirtualProductGrid = VirtualGridComponent;
      isLoaded = true;
  ...
  // Search function using remote functions
  ...
  onMount(() => {
    // Create lazy loader that triggers when component comes into view
    const lazyLoader = createLazyLoader(loadComponents, { rootMargin: '100px' });
    return lazyLoader(container);
  });
</script>

<div bind:this={container} class="lazy-search-results">
  {#if !isLoaded}
    <!-- Loading placeholder -->
