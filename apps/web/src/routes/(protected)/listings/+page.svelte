<script lang="ts">
  ...
  const filteredListings = $derived(() => {
    ...
    if (activeTab === 'draft') return data.listings.filter(l => l.status === 'draft');
  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
      ...
      <div class="bg-white p-4 rounded-lg shadow-xs">
        ...
        <p class="text-2xl font-bold text-blue-600">{data.stats.active}</p>
      </div>
      <div class="bg-white p-4 rounded-lg shadow-xs">
        <p class="text-sm text-gray-600">{i18n.listings_sold()}</p>
        <p class="text-2xl font-bold text-green-600">{data.stats.sold}</p>
      </div>
      <div class="bg-white p-4 rounded-lg shadow-xs">
        <p class="text-sm text-gray-600">{i18n.listings_draft()}</p>
        <p class="text-2xl font-bold text-yellow-600">{data.stats.draft}</p>
      </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-xs mb-6">
      <nav class="flex space-x-8 px-6 py-4">
        ...
        >
          {i18n.listings_all()} ({data.stats.total})
        </button>
        <button
          ...
          class="text-sm font-medium {activeTab === 'active' 
            ...
            : 'text-gray-500 hover:text-gray-700'}"
        >
          {i18n.listings_active()} ({data.stats.active})
        </button>
        <button
          ...
          class="text-sm font-medium {activeTab === 'sold' 
            ...
            : 'text-gray-500 hover:text-gray-700'}"
        >
          {i18n.listings_sold()} ({data.stats.sold})
        </button>
        <button
          ...
          class="text-sm font-medium {activeTab === 'draft' 
            ...
            : 'text-gray-500 hover:text-gray-700'}"
        >
          {i18n.listings_draft()} ({data.stats.draft})
        </button>
      </nav>
    </div>
              ...
              <div class="bg-white rounded-lg shadow-xs overflow-hidden hover:shadow-md transition-shadow">
                ...
                <div class="aspect-square bg-gray-100">
                  {#if listing.images && listing.images.length > 0}
                    <img
                      src={listing.images[0]?.image_url || '/placeholder-product.svg'}
                      alt={listing.title}
