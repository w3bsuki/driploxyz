...
async function buildOptimized(): Promise<void> {
  ...
  const locales: string[] = messageFiles
    .filter(file => file.endsWith('.json'))
    .map(file => path.basename(file, '.json'));

  // Load all messages for each locale
  const allMessages: MessageCollection = {};
...
if (typeof window !== 'undefined') {
  ...
  loadMessages(_currentLocale).catch(
}`;
}

function generateLocaleBundle(locale: string, messages: Record<string, string>): string {
...
function generateOptimizedIndex(locales: string[], allMessages: MessageCollection): string {
...
function createLazyMessage(key: string, fallback: string) {
  let messageFunc: ((inputs?: Record<string, string>, options?: any) => Promise<string>) | null = null;

  return function(inputs: Record<string, string> = {}, options: any = {}) {
    if (!messageFunc) {
...
export function getMessage(key: string, fallback?: string) {
...
function generateTypes(allMessages: MessageCollection): string {
