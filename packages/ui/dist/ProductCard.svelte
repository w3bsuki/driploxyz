...
<Card class={`overflow-hidden ${className}`} padding={false} hover={true} onclick={handleClick}>
  ...
  <div class="p-3">
    <div class="flex items-start justify-between gap-3 mb-3">
      ...
      <div class="bg-gray-900 text-white px-2 py-1 rounded-lg">
    ...
    <div class="space-y-2">
      ...
      <div class="flex items-center gap-2 pt-2 border-t border-gray-100">
        <Avatar 
          ...
          size="xs" 
        />
        <span class="truncate text-xs text-gray-600 font-medium flex-1">{product.sellerName || translations.unknownSeller}</span>
        
        {#if product.sellerRating !== undefined && product.sellerRating !== null}
          <div class="flex items-center gap-1 bg-amber-50 px-2 py-1 rounded-full flex-shrink-0">
        ...
        {:else}
          <span class="text-[10px] bg-blue-100 text-blue-700 px-2 py-1 rounded-full font-semibold flex-shrink-0">
