...
const activeToasts = new Map<string, string>(); // hash -> toastId
...
function createToastStore(): ToastStore {
  // Reactive toasts state using writable store for test compatibility
  ...
  function addToast(toast: Toast): string {
    ...
    if (activeToasts.has(toastHash)) {
      // Duplicate detected - return existing toast ID without creating new toast
      return activeToasts.get(toastHash)!;
    }

    // Get the actual ID that will be used (provider ID or generated ID)
    let actualToastId: string;

    if (toastProvider && typeof toastProvider.addToastData === 'function') {
      actualToastId = toastProvider.addToastData(toast);
    } else {
      ...
      actualToastId = toast.id;
    }

    // Mark as active and store the actual ID that was mounted
    activeToasts.set(toastHash, actualToastId);
