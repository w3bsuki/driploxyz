  ...
  <div class="border rounded-xl p-4 {conditionData.bgColor} {conditionData.borderColor}">
    <div class="flex items-start justify-between mb-3">
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-gray-600 mt-0.5">{conditionData.description}</p>
        </div>
      </div>
    ...
    <div class="grid grid-cols-2 gap-2 text-sm">
      {#each conditionDetails.slice(0, 2) as detail, index}
        ...
        >
          <span class="text-gray-700">{detail.aspect}:</span>
        ...
        {#each conditionDetails as detail, index}
          ...
          >
            <div class="flex-1">
              ...
              {#if detail.notes}
                <p class="text-xs text-gray-600 mt-0.5">{detail.notes}</p>
              {/if}
            </div>
            <div class="flex items-center gap-1 ml-4">
              ...
              {/each}
              <span class="text-sm font-medium text-gray-600 ml-1">{detail.rating}/5</span>
            </div>
          </div>
        {/each}
      </div>
