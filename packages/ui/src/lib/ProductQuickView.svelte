	...
	>
		<div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
			...
			<div class="relative inline-block w-full max-w-4xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-sm md:shadow-2xl transition-colors">
				<button
					onclick={onClose}
					class="absolute right-4 top-4 z-10 rounded-full bg-white/90 p-2 text-gray-500 md:backdrop-blur-sm transition-colors hover:bg-white hover:text-gray-900 hover:shadow-sm md:hover:shadow-lg"
					aria-label="Close"
				>
					...
					<div class="flex flex-col p-6 md:p-8">
						<div class="flex-1">
							...
							{#if product?.description}
								<p class="mb-6 text-gray-500 line-clamp-3">
									{product?.description}
								</p>
							{/if}
							...
							<div class="mb-6 space-y-3">
								{#if product?.size}
									<div class="flex items-center gap-2">
										...
										<span class="rounded-full bg-gray-100 px-3 py-1 text-sm font-medium text-gray-900">
											{product?.size}
										</span>
									</div>
								{/if}
						...
						<div class="mt-6 flex gap-3">
							...
							<button
								onclick={onView}
								class="flex flex-1 items-center justify-center gap-2 rounded-xl border-2 border-gray-200 bg-white px-6 py-3 font-semibold text-gray-900 transition-colors hover:border-gray-300 hover:bg-gray-50"
							>
								<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							...
							<button
								...
								class="flex items-center justify-center rounded-xl border-2 border-gray-200 bg-white p-3 text-gray-500 transition-colors hover:border-red-200 hover:bg-red-50 hover:text-red-500"
