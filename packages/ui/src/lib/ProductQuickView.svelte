	...
	>
		<div class="flex min-h-screen items-center justify-center p-4 text-center sm:p-0">
			...
			<div class="relative inline-block w-full max-w-4xl transform overflow-hidden rounded-2xl bg-white text-left align-middle shadow-sm md:shadow-2xl transition-colors">
				<button
					onclick={onClose}
					class="absolute right-4 top-4 z-10 rounded-full bg-white/90 p-2 text-[color:var(--text-muted)] md:backdrop-blur-sm transition-colors hover:bg-white hover:text-[color:var(--text-primary)] hover:shadow-sm md:hover:shadow-lg"
					aria-label="Close"
				>
				...
				<div class="grid md:grid-cols-2">
					...
					<div class="flex flex-col p-6 md:p-8">
						<div class="flex-1">
							<div class="mb-2 flex items-center gap-2 text-sm text-[color:var(--text-muted)]">
								<span class="font-medium">{product?.brand || ''}</span>
								{#if product?.condition}
									<span class="text-[color:var(--text-disabled)]">•</span>
									<span>{product?.condition}</span>
								{/if}
							</div>

							<h3 id="modal-title" class="mb-4 text-2xl font-bold text-[color:var(--text-primary)]">
								{product?.title || ''}
							</h3>

							<div class="mb-6 flex items-baseline gap-3">
								<span class="text-3xl font-bold text-[color:var(--text-primary)]">
									{formatCurrency(product?.price || 0)}
								</span>
								{#if product?.originalPrice && product?.originalPrice > (product?.price || 0)}
									<span class="text-lg text-[color:var(--text-disabled)] line-through">
							...
							{#if product?.description}
								<p class="mb-6 text-[color:var(--text-muted)] line-clamp-3">
									{product?.description}
								</p>
							{/if}
							...
							<div class="mb-6 space-y-3">
								{#if product?.size}
									<div class="flex items-center gap-2">
										<span class="text-sm font-medium text-[color:var(--text-muted)]">Size:</span>
										<span class="rounded-full bg-gray-100 px-3 py-1 text-sm font-medium text-[color:var(--text-primary)]">
											{product?.size}
										</span>
									</div>
								{/if}
								...
								<div class="flex flex-wrap gap-2">
									<div class="flex items-center gap-1.5 text-sm text-[color:var(--text-muted)]">
									...
									<div class="flex items-center gap-1.5 text-sm text-[color:var(--text-muted)]">
										<svg class="h-4 w-4 text-[color:var(--text-link)]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							...
							{#if product?.sellerName}
								<div class="flex items-center gap-3 border-t pt-6">
									<div class="flex items-center gap-2">
										...
										{/if}
										<div>
											<p class="text-sm font-medium text-[color:var(--text-primary)]">{product.sellerName || 'Seller'}</p>
											{#if product.sellerRating}
												<div class="flex items-center gap-1">
													<span class="text-xs text-yellow-500">★</span>
													<span class="text-xs text-[color:var(--text-muted)]">{product.sellerRating}</span>
												</div>
											{/if}
										</div>
									</div>
						...
						<div class="mt-6 flex gap-3">
							...
							<button
								onclick={onView}
								class="flex flex-1 items-center justify-center gap-2 rounded-xl border-2 border-gray-200 bg-white px-6 py-3 font-semibold text-[color:var(--text-primary)] transition-colors hover:border-gray-300 hover:bg-gray-50"
							>
								<svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
							...
							<button
								...
								class="flex items-center justify-center rounded-xl border-2 border-gray-200 bg-white p-3 text-[color:var(--text-muted)] transition-colors hover:border-red-200 hover:bg-red-50 hover:text-red-500"
