<script lang="ts">
  ...
  const baseClasses = 'block w-full rounded-lg border-2 border-gray-200 px-3 py-2 text-sm placeholder-gray-500 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-gray-500/20 focus:border-gray-500 pl-8';
  const stateClasses = $derived(error 
    ? 'border-[var(--status-error-solid)] focus:ring-[var(--status-error-solid)]/20 focus:border-[var(--status-error-solid)]' 
    : '');
