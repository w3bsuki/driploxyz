...
<div class="relative {className}">
  <form onsubmit={handleSubmit}>
    <div class="relative flex items-center {variant === 'hero' ? 'bg-white rounded-full border-2 border-gray-200 shadow-sm hover:shadow-md focus-within:border-black transition-all' : 'bg-white rounded-lg border-2 border-gray-200 focus-within:border-black transition-colors'}">
      <!-- Search Icon -->
      ...
      <input
        ...
        {placeholder}
        type="search"
        class="w-full bg-transparent pl-12 {variant === 'hero' ? 'pr-24' : 'pr-32'} py-3 text-sm placeholder-gray-500 focus:outline-none"
        oninput={handleInput}
        onfocus={handleFocus}
        onblur={handleBlur}
      />
      ...
      {#if variant === 'hero'}
        <button
          type="button"
          onclick={onFilter}
          class="absolute right-2 px-3 py-1.5 bg-white rounded-full hover:bg-gray-50 transition-colors flex items-center space-x-1 border border-gray-200"
        >
