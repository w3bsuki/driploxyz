      ...
      <div class="pt-1 border-t border-gray-100">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-1.5 min-w-0">
            ...
            />
            <div class="min-w-0">
              <span class="block truncate text-xs text-gray-600">{product.sellerName || translations.unknownSeller}</span>
              {#if product.sellerRating !== undefined && product.sellerRating !== null}
                <div class="flex items-center gap-0.5 mt-0.5">
                  ...
                  <span class="text-[11px] font-medium text-gray-700">{product.sellerRating.toFixed(1)}</span>
                </div>
              {:else}
                <span class="inline-block text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded font-medium mt-0.5">
          ...
          <!-- Price moved to seller row -->
          <span class="text-lg font-bold text-gray-900 flex-shrink-0">{translations.formatPrice ? translations.formatPrice(product.price) : `${translations.currency}${typeof product.price === 'number' ? product.price.toFixed(2) : product.price}`}</span>
        </div>
      </div>
    </div>
  </div>
